...
```{r}
inv = read.csv('dados/cauaxi.csv')
inv = inv %>% filter(!is.na(ida))
```


O orgão, ficou com dúvida sobre o valor e decidiu comparar com os valores obtidos por um trabalho de mestrado realizado numa área de floresta na região de Paragominas. Neste trabalho, o incremente médio diamétrico foi de `mean(na.omit(inv$ida))` cm/ano e um desvio de `sd(na.omit(inv$ida))`.
...
O analista deseja saber se o valores de incremento apresentado pela empresa está dentro do que se conhece sobre a área. Para isto, ele vai usar a estatisticas Bayesiama para analisar os dados.
